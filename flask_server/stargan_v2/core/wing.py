...
class AddCoordsTh(nn.Module):
    def __init__(self, height=64, width=64, with_r=False, with_boundary=False):
        ...
        with torch.no_grad():
            x_coords = torch.arange(height).unsqueeze(
                1).expand(height, width).float()
            y_coords = torch.arange(width).unsqueeze(
                0).expand(height, width).float()
    ...
    def forward(self, x, heatmap=None):
        ...
        if self.with_boundary and heatmap is not None:
            ...
            xx_boundary_channel = torch.where(
                boundary_channel > 0.05, self.x_coords, zero_tensor).to(zero_tensor.device)
            yy_boundary_channel = torch.where(
                boundary_channel > 0.05, self.y_coords, zero_tensor).to(zero_tensor.device)
            coords = torch.cat(
...
class ConvBlock(nn.Module):
    def __init__(self, in_planes, out_planes):
        ...
        conv3x3 = partial(nn.Conv2d, kernel_size=3, stride=1,
...
class FaceAligner():
    ...
    def align(self, imgs, output_size=256):
        ...
        for i, (img, landmarks) in enumerate(zip(imgs, landmarkss)):
            ...
            print("Transform matrix calculated")
